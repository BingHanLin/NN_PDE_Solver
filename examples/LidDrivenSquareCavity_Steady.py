...
if __name__ == "__main__":
    ...
    # boundary dataset
    bc_x = np.concatenate((np.zeros(40),
                          np.linspace(0.0, 1.0, num=40),
                          np.ones(40),
                          np.linspace(0.0, 1.0, num=40)))
    bc_y = np.concatenate((np.linspace(0.0, 1.0, num=40),
                           np.ones(40),
                           np.linspace(0.0, 1.0, num=40),
                           np.zeros(40)))
    bc_input = np.stack((bc_x, bc_y), -1)

    bc_u = np.concatenate((np.zeros(40),
                           np.ones(40),
                           np.zeros(40),
                           np.zeros(40)))
    bc_v = np.concatenate((np.zeros(40),
                           np.zeros(40),
                           np.zeros(40),
                           np.zeros(40)))
    ...
    solver.solve(2, 250, model, optimizer, device)
